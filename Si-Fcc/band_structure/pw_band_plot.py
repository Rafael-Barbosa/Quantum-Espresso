...
def pw_band_plot():
        ...
        for i in range(nvband):
            line1=plt.plot( eig[:,i]-e_ref,color='b',linewidth=lw )
        for j in range(nvband, nbnd):
            line2=plt.plot( eig[:,j]-e_ref,color='r',linewidth=lw )
        ...
        plt.text(0.0, 10.5, 'Silicon FCC', fontsize=12, color='black')

        plt.savefig('pwband.png',dpi=300)
        plt.show()
